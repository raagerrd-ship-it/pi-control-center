...
build_status_json() {
  ...
  # Dashboard API + Nginx stats (both on core 0)
  local dash_cpu=0 dash_ram=0
  dash_ram=$(get_service_ram "pi-dashboard-api")
  local nginx_ram=$(get_service_ram "nginx")
  dash_ram=$((dash_ram + nginx_ram))
  local dash_pid=$(systemctl show "pi-dashboard-api.service" --property=MainPID 2>/dev/null | cut -d= -f2)
...
handle_request() {
  ...
  case "$method $path" in
    ...
    "GET /api/versions")
      # Check remote HEAD for each app + dashboard — use dates
      local vj=""
      for app in lotus-lantern cast-away sonos-gateway; do
        local dir=${APP_DIRS[$app]} repo=${APP_REPOS[$app]}
        local local_v="" local_hash="" remote_hash=""
        if [ -d "$dir/.git" ]; then
      ...
      # Dashboard
      local d_local="" d_hash="" d_remote_hash=""
