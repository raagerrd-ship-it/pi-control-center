...
auto_adjust_memory_limit() {
  ...
  _app_set_limit "$app" "$next_mb" || live_change=0
  sudo_run_quiet systemctl daemon-reload || user_systemctl daemon-reload 2>/dev/null || true
  # Hoppa över try-restart om set-property uppdaterade cgroup live —
  # restart skulle riva ner BLE/audio-länkar i onödan.
  [ "$live_change" -eq 1 ] || _app_try_restart "$app"
  rm -f "$CACHE_FILE"
  memory_autoscale_write "$app" 0 0 "$now"
  if [ "$live_change" -eq 1 ]; then
    append_memory_change_log "$app" "MEMORY: ${app} MemoryMax ${old_limit}MB → ${next_mb}MB (${current_level} → ${next_level}) efter ${ram}MB/${old_limit}MB (${pct}%) [live, ingen restart]"
  else
    append_memory_change_log "$app" "MEMORY: ${app} MemoryMax ${old_limit}MB → ${next_mb}MB (${current_level} → ${next_level}) efter ${ram}MB/${old_limit}MB (${pct}%) [restart krävdes]"
  fi
}
...
# Returnerar 0 om alla set-property lyckades live (cgroup uppdaterad utan
# omstart), 1 om någon set-property failade (anroparen behöver göra
# daemon-reload + try-restart för att få igenom det nya MemoryMax).
_app_set_limit() {
  local app="$1" limit_mb="$2"
  local s f all_live=1
  if [ "$(registry_has_components "$app")" = "true" ]; then
    for comp in engine ui; do
      ...
      if ! sudo_run_quiet systemctl set-property "${s}.service" "MemoryMax=${limit_mb}M"; then
        if ! user_systemctl set-property "${s}.service" "MemoryMax=${limit_mb}M" 2>/dev/null; then
          all_live=0
        fi
      fi
    done
...
# Skriv nginx vhost för en apps UI om appen har en dist-katalog.
# Returnerar 0 om vhost skrevs/uppdaterades, 1 annars.
_app_write_nginx_vhost() {
  local app="$1"
  local install_dir port dist_dir vhost_file tmp core engine_port
  install_dir=$(eval echo "$(registry_get "$app" "installDir")")
  port=$(registry_get "$app" "port")
  [ -n "$install_dir" ] || return 1
  [ -n "$port" ] || return 1
  dist_dir="$install_dir/dist"
  [ -d "$dist_dir" ] || return 1

  # Engine-port härleds från appens core-assignment (3050 + core).
  # Krävs för /api/ + /events same-origin-proxy så att UI:er fungerar
  # från vilken enhet som helst (mobil, laptop) utan hardcoded 127.0.0.1.
  core=$(assignment_get_core "$app")
  if [ -n "$core" ] && [ "$core" -gt 0 ] 2>/dev/null; then
    engine_port=$(engine_port_for_core "$core")
  else
    engine_port=""
  fi

  vhost_file="/etc/nginx/sites-available/${app}-ui"
  tmp=$(mktemp)
  {
    cat <<EOF
# ${app} UI — auto-generated by Pi Control Center
server {
    listen ${port};
    listen [::]:${port};
    server_name _;
...
EOF
    if [ -n "$engine_port" ]; then
      cat <<EOF
    # Rewrite hardcoded 127.0.0.1:305x i bundled JS så UI:t fungerar
    # från vilken enhet som helst (mobil, laptop) — inte bara på Pi:n själv.
    sub_filter_once off;
    sub_filter_types application/javascript text/javascript;
    sub_filter '127.0.0.1:3051' '\$host:3051';
    sub_filter '127.0.0.1:3052' '\$host:3052';
    sub_filter '127.0.0.1:3053' '\$host:3053';
...
EOF
    ...
    location ~* \.(js|css|woff2?|png|jpg|jpeg|gif|ico|svg|webp|map)$ {
        expires 30d;
...
EOF
  ...
  sudo_run install -m 0644 -o root -g root "$tmp" "$vhost_file"
  rm -f "$tmp"
  sudo_run ln -sf "$vhost_file" "/etc/nginx/sites-enabled/${app}-ui"
  if sudo_run /usr/sbin/nginx -t 2>/dev/null; then
    sudo_run /usr/sbin/nginx -s reload || true
    return 0
  else
    log "NGINX: config test failed for ${app}, rolling back"
    sudo_run rm -f "/etc/nginx/sites-enabled/${app}-ui"
    return 1
  fi
}

# Replace the app's *-ui.service ExecStart with a no-op so it doesn't
# bind the port that nginx now owns, but stays "active" so PCC's
# component health check passes.
_app_neutralize_ui_unit() {
  local app="$1" s dropin_dir dropin_file
  if [ "$(registry_has_components "$app")" = "true" ]; then
    s=$(registry_get_component "$app" "ui" "service")
  else
    return 0
  fi
  [ -n "$s" ] || return 0
  dropin_dir="/etc/systemd/system/${s}.service.d"
  dropin_file="${dropin_dir}/nginx-takeover.conf"
  sudo_run mkdir -p "$dropin_dir"
  sudo_run tee "$dropin_file" > /dev/null <<'EOF'
# UI is now served by nginx (sites-enabled/<app>-ui).
# Keep the unit active so PCC dashboard reports component online,
# but make it a no-op so it doesn't bind the port nginx owns.
[Service]
ExecStart=
...
EOF
  ...
  sudo_run_quiet systemctl restart "${s}.service" || true
}
